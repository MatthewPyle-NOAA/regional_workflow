...
#
#  Usage: rhist_savefv3ccpp.sh Directory Date(YYYYMMDDHH format)
#
...
if [ $# -ne 2 ]
then
  echo "Usage: rhist_savefv3ccpp.sh Directory Date(YYYYMMDDHH format) "
  exit 1
fi 
...
if [ ! -d $dir ]
then
  echo "rhist_savefv3ccpp.sh:  Directory $dir does not exist."
  exit 2
fi 
...
hpssdir0=/NCEPDEV/emc-meso/2year/${USER}/fv3ccpp/rh${year}/${yearmo}/$yrmoday
hpssdir1=/NCEPDEV/emc-meso/2year/${USER}/fv3ccpp/rh${year}/${yearmo}/$yrmoday
hpssdir2=/NCEPDEV/emc-meso/2year/${USER}/fv3ccpp/rh${year}/${yearmo}/$yrmoday
...
do
   ...
   tarfile=fv3ccpp.${yrmoday}${rhcyc}.${file}.tar
   ...
   if [ $err -ne 0 ]
   then
     echo "rhist_savefv3ccpp.sh:  File $tarfile was not successfully created."
     exit 3
   fi
   date
   ...
   if [ $err -ne 0 ]
   then
     echo "rhist_savefv3ccpp.sh:  Tar file $tarfile was not successfully read to"
