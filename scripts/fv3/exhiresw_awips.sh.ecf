...
then
  ...
  do
    if [ -f $COMIN/${NET}.t${cyc}z.${MODEL}_5km.f${fhr}.conus.grib2_sbn.idx ] ; then
      break
    else
      let "icnt=icnt+1"
      sleep 30
  ...
  if [ $icnt -ge $maxtries ]; then
    ...
    export FORT11=$COMIN/${NET}.t${cyc}z.${MODEL}_5km.f${fhr}.conus.grib2_sbn
    export FORT31=""
    export FORT51=grib2.t${cyc}z.awpreg_${RUNLOC}f${fhr}
    $TOCGRIB2 < $PARMutil/grib2_${NEST}_hiresw.awpreg_f${fhr}

    if [ $SENDCOM = "YES" ] ; then
       cp  $FORT51  $COMOUT/wmo/
    fi

    if [ $SENDDBN_NTC = "YES" ] ; then
       $DBNROOT/bin/dbn_alert NTC_LOW ${DBN_ALERT_TYPE} $job $COMOUT/wmo/grib2.t${cyc}z.awpreg_${RUNLOC}f${fhr}
    else
