...
do
 ...
 if [ $loop -ge $looplim ]
   then
# expanding limit due to Cray problems with jobs not running
   msg="FATAL ERROR: ABORTING after 30 minutes of waiting for $INPUT_DATA/logf0${fhr}"
   err_exit $msg
 fi
done
...
if [ -e $filecheck ]
then
$EXECfv3/hireswfv3_fv3snowbucket.x < input
export err=$?; err_chk
...
then
 ...
 $EXECfv3/hireswfv3_bucket.x < input.card
 cat ./PCP3HR${fhr}.tm00 >> $filecheck
