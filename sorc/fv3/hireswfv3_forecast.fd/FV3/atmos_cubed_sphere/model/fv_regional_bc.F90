...
!--------------------------
!
      if (data_source == 'FV3GFS GAUSSIAN NEMSIO FILE' .or.             &
          data_source == 'FV3GFS GAUSSIAN NETCDF FILE') then
        nlev=klev_in
        var_name_root='t'
        call read_regional_bc_file(is_input,ie_input,js_input,je_input  &
                                  ,nlev                                 &
...
! Compute true temperature using hydrostatic balance if not read from input.
        ...
        if (data_source /= 'FV3GFS GAUSSIAN NEMSIO FILE' .and. &
            data_source /= 'FV3GFS GAUSSIAN NETCDF FILE') then
          do k=1,npz
...
!--- Adjust delp with tracer mass.
     ...
     enddo
!
   else source   ! This else block is for all sources other than FV3GFS GAUSSIAN NEMSIO/NETCDF FILE
!
