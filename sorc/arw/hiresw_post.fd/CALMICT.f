...
!
     ...
     &  NSImax=250.E3, NLI_min=1.0E3
      ...
      real TC, Frain,Fice,Flimass,FLARGE,          &
           ...
           DRmm,Qsigrd,WVQW,Dum,XLi,Qlice,WC,DLI,xlimass,NLImax,NSI_max
      real,external :: fpvs
      logical :: HAIL, LARGE_RF
!************************************************************************
...
!
              NSI_max=MAX(NSImax, 0.1*RHO*QICE/MASSI(MDImin) )

              NSmICE=MIN(0.01*EXP(-0.6*TC), NSI_max)       !- Fletcher (1962)
...
!
!tst                IF (NLICE>=NLImax) RimeF=DUM/(NLImax*MASSI(INDEXS))
              ENDIF             ! End IF (XLI .LE. MASSI(MDImin) )
no_hail:    IF (.NOT. HAIL) THEN
              NLICE=DUM/(RimeF*MASSI(INDEXS))     !-- NLICE > NLImax
            ENDIF    no_hail
